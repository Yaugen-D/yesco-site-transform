...
const Hero = () => (
  <section className="relative flex min-h-screen flex-col justify-between px-6 pb-16 pt-8 md:px-12 lg:px-20">
    ...
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="mt-auto flex max-w-5xl flex-col gap-8 pt-32"
    >
      ...
      <motion.h1
        variants={fadeUp}
        className="text-5xl font-extrabold leading-[1.05] tracking-tighter text-foreground md:text-7xl lg:text-8xl"
      >
        Platform Stability
        <br />
      ...
      <motion.p
        variants={fadeUp}
        className="max-w-[48ch] text-lg text-muted-foreground md:text-xl"
      >
    ...
    >
      {industries.map((ind) => (
        <motion.span
          key={ind}
          variants={fadeUp}
          className="rounded-full border border-border px-4 py-1.5 font-mono-label text-muted-foreground"
        >
          {ind}
        </motion.span>
      ))}
