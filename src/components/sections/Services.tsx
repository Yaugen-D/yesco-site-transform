...
const packages = [
  ...
  },
  {
    name: "Scale",
    type: "Project-based",
    desc: "Architecture for growth",
    deliverables: [
      "Architecture review & redesign",
      "Auto-scaling implementation",
      "CI/CD pipeline optimization",
      "Load testing & performance tuning",
      "Migration & zero-downtime deployments",
    ],
    ...
    >
      {packages.map((pkg) => (
        <motion.div
          key={pkg.name}
          variants={fadeUp}
          className="flex flex-col rounded-2xl bg-card p-8 shadow-[0_1px_3px_0_hsl(var(--foreground)/0.04),0_8px_24px_-4px_hsl(var(--foreground)/0.06)] transition-transform duration-300 hover:-translate-y-1"
        >
          ...
          <p className="mt-8 border-t border-border pt-6 text-sm font-medium text-muted-foreground">
            {pkg.result}
          </p>
        </motion.div>
      ))}
